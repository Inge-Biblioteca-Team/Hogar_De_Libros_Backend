...
export class FurnitureService {
  ...
  async create(
    ...
    try {
      ...
      this.FurnitureRepository.save(newFurniture);
  ...
  async NAFurniture(Id: number): Promise<{message: string}> {
    try{
      ...
      const errorMessage =
        (error as Error).message || 'Error al cambiar el estado del mobiliario a N.A.';
      throw new InternalServerErrorException(errorMessage);
    }
  }
  ...
  async SEFurniture(Id: number): Promise<{message: string}> {
    try{
      ...
      const errorMessage =
        (error as Error).message || 'Error al cambiar el estado del mobiliario a S.E.';
      throw new InternalServerErrorException(errorMessage);
    }
  }
